...
var searchreplace = (function () {
  ...
  var $_d9uo28iijctdj3eh = { findAndReplaceDOMText: findAndReplaceDOMText };
  ...
  var markAllMatches = function (editor, currentIndexState, regex) {
    ...
    return $_d9uo28iijctdj3eh.findAndReplaceDOMText(regex, node, marker, false, editor.schema);
  };
  ...
  var get = function (editor, currentIndexState) {
    var done = function (keepEditorSelection) {
      return $_4x5sbzigjctdj3ea.done(editor, currentIndexState, keepEditorSelection);
    };
    var find = function (text, matchCase, wholeWord) {
      return $_4x5sbzigjctdj3ea.find(editor, currentIndexState, text, matchCase, wholeWord);
    };
    var next = function () {
      return $_4x5sbzigjctdj3ea.next(editor, currentIndexState);
    };
    var prev = function () {
      return $_4x5sbzigjctdj3ea.prev(editor, currentIndexState);
    };
    var replace = function (text, forward, all) {
      return $_4x5sbzigjctdj3ea.replace(editor, currentIndexState, text, forward, all);
    };
    return {
      done: done,
      find: find,
  ...
  var open = function (editor, currentIndexState) {
    ...
    function updateButtonStates() {
      win.statusbar.find('#next').disabled($_4x5sbzigjctdj3ea.hasNext(editor, currentIndexState) === false);
      win.statusbar.find('#prev').disabled($_4x5sbzigjctdj3ea.hasPrev(editor, currentIndexState) === false);
    }
    ...
    var win = editor.windowManager.open({
      ...
      onClose: function () {
        editor.focus();
        $_4x5sbzigjctdj3ea.done(editor, currentIndexState);
        editor.undoManager.add();
      },
      onSubmit: function (e) {
        ...
        if (!text.length) {
          $_4x5sbzigjctdj3ea.done(editor, currentIndexState, false);
          win.statusbar.items().slice(1).disabled(true);
          return;
        }
        if (last.text === text && last.caseState === caseState && last.wholeWord === wholeWord) {
          if (!$_4x5sbzigjctdj3ea.hasNext(editor, currentIndexState)) {
            notFoundAlert();
            return;
          }
          $_4x5sbzigjctdj3ea.next(editor, currentIndexState);
          updateButtonStates();
          return;
        }
        count = $_4x5sbzigjctdj3ea.find(editor, currentIndexState, text, caseState, wholeWord);
        if (!count) {
          notFoundAlert();
        }
      ...
      buttons: [
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            if (!$_4x5sbzigjctdj3ea.replace(editor, currentIndexState, win.find('#replace').value())) {
              win.statusbar.items().slice(1).disabled(true);
              currentIndexState.set(-1);
              last = {};
            }
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_4x5sbzigjctdj3ea.replace(editor, currentIndexState, win.find('#replace').value(), true, true);
            win.statusbar.items().slice(1).disabled(true);
            last = {};
          }
        },
        ...
        },
        {
          ...
          disabled: true,
          onclick: function () {
            $_4x5sbzigjctdj3ea.prev(editor, currentIndexState);
            updateButtonStates();
          }
        },
        {
  ...
  var $_baequgikjctdj3en = { open: open };

  var register = function (editor, currentIndexState) {
    editor.addCommand('SearchReplace', function () {
      $_baequgikjctdj3en.open(editor, currentIndexState);
    });
  };
  var $_7q0idfijjctdj3el = { register: register };
  ...
  var $_2v9uxxiljctdj3eq = { register: register$1 };

  PluginManager.add('searchreplace', function (editor) {
    var currentIndexState = Cell(-1);
    $_7q0idfijjctdj3el.register(editor, currentIndexState);
    $_2v9uxxiljctdj3eq.register(editor, currentIndexState);
    return $_253esaifjctdj3e5.get(editor, currentIndexState);
  });
  var Plugin = function () {
  };
