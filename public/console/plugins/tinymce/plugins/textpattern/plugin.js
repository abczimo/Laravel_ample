...
var textpattern = (function () {
  ...
  var $_762tvnq4jctdj53g = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_dn1e1sqcjctdj53s.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_dn1e1sqcjctdj53s.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_dn1e1sqcjctdj53s.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_1uopclq9jctdj53o.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_1uopclq9jctdj53o.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_1uopclq9jctdj53o.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_givel6q8jctdj53l.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_givel6q8jctdj53l.checkKeyCode(keyCodes, e)) {
        $_givel6q8jctdj53l.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_givel6q8jctdj53l.checkCharCode(charCodes, e)) {
  ...
  var $_dbzwfeq5jctdj53i = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_762tvnq4jctdj53g.getPatterns(editor.settings));
    $_dbzwfeq5jctdj53i.setup(editor, patternsState);
    return $_48uvlqq3jctdj53f.get(patternsState);
  });
  var Plugin = function () {
  };
